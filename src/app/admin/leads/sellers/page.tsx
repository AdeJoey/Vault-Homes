...
export default function SellersList() {
  ...
  useEffect(() => {
    fetch("/api/admin/leads?type=SELLER")
      .then(res => res.json())
      .then(data => {
        setLeads(data.leads || []);
        setLoading(false);
  ...
  return (
    <div className="pb-12">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Seller Leads</h1>
          <p className="text-gray-500 mt-1">Manage and match property owners.</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50/80 text-gray-500 text-xs font-semibold uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-5 border-b border-gray-100">Location</th>
                <th className="px-6 py-5 border-b border-gray-100">Type</th>
                <th className="px-6 py-5 border-b border-gray-100">Expected Price</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {leads.map((lead) => (
                <tr key={lead.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-gray-600">{lead.propertyLocation || "-"}</td>
                  <td className="px-6 py-4 text-gray-600">{lead.propertyType || "-"}</td>
                  <td className="px-6 py-4 text-gray-900 font-medium">{lead.priceExpectation || "-"}</td>
