"use client";

import { useState } from "react";
import { Button } from "@/components/ui/Button";

export function SellForm() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle");

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setStatus("loading");
    const form = e.currentTarget;
    const data = Object.fromEntries(new FormData(form));
    try {
      const res = await fetch("/api/leads/seller", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      if (!res.ok) throw new Error();
      setStatus("success");
      form.reset();
    } catch {
      setStatus("error");
    }
  }

  return (
    <section className="bg-white min-h-screen py-24 px-4 md:px-16 w-full flex flex-col justify-center">
      <div className="w-[90%] max-w-2xl mx-auto">
        <div className="mb-12">
          <h2 className="text-[40px] md:text-[52px] font-bold text-black leading-tight tracking-[-0.02em] mb-4">
            Start your<br />private review.
          </h2>
          <p className="text-[16px] text-gray-600">
            Fill in the details below and our team will be in touch within 24 hours.
          </p>
        </div>

        {status === "success" ? (
          <div className="bg-[#E0E3EA] rounded-[24px] p-10 text-center flex flex-col items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-[#E5ED64] flex items-center justify-center text-3xl">✓</div>
            <h3 className="text-[24px] font-bold text-black">Submission received!</h3>
            <p className="text-gray-600">Our team will review your property and reach out privately within 24–48 hours.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {/* Name */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Full Name</label>
              <input
                name="name"
                required
                placeholder="e.g. Adebayo Okafor"
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors"
              />
            </div>
            {/* Phone */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Phone Number</label>
              <input
                name="phone"
                required
                placeholder="+234 800 000 0000"
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors"
              />
            </div>
            {/* Email */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Email Address</label>
              <input
                name="email"
                type="email"
                required
                placeholder="you@email.com"
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors"
              />
            </div>
            {/* Location */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Property Location</label>
              <input
                name="location"
                required
                placeholder="e.g. Ikoyi, Lagos"
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors"
              />
            </div>
            {/* Property Type */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Property Type</label>
              <select
                name="propertyType"
                required
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors bg-white"
              >
                <option value="">Select type</option>
                <option>Detached House</option>
                <option>Semi-Detached House</option>
                <option>Duplex</option>
                <option>Apartment / Flat</option>
                <option>Land / Plot</option>
                <option>Commercial Property</option>
              </select>
            </div>
            {/* Asking Price */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Asking Price (₦)</label>
              <input
                name="price"
                required
                placeholder="e.g. 250,000,000"
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors"
              />
            </div>
            {/* Notes */}
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-semibold text-gray-700">Additional Notes (optional)</label>
              <textarea
                name="notes"
                rows={4}
                placeholder="Any specific preferences or context..."
                className="border border-gray-300 rounded-[14px] px-5 py-4 text-[15px] outline-none focus:border-[#E5ED64] transition-colors resize-none"
              />
            </div>

            {status === "error" && (
              <p className="text-red-500 text-[14px]">Something went wrong. Please try again or WhatsApp us directly.</p>
            )}

            <Button
              type="submit"
              variant="gradient"
              disabled={status === "loading"}
              className="h-[56px] text-[15px] font-semibold rounded-full mt-2 w-full"
            >
              {status === "loading" ? "Submitting…" : "Submit for Private Review"}
            </Button>
          </form>
        )}
      </div>
    </section>
  );
}
